  ...
  <tr key={item.id} className="text-gray-900 text-xs text-start rounded-lg">
    {column.map((columnItem: any, index: number) => {
      if (columnItem.value.includes(".")) {
        ...
        return <td className="px-6">{item[itemSplit[0]][itemSplit[1]]}</td>;
      }
          ...
          {typeof item[`${columnItem.value}`] === "boolean" ? (
            <span
              className={classNames(
                "bg-red-100 text-red-800 text-xs text-start font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-red-200 dark:text-red-900",
                {
                  "bg-green-100 text-green-800 text-xs font-semibold mr-2 px-2.5 rounded dark:bg-green-200 dark:text-green-900":
